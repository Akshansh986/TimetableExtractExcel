...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// if (true) return;
		SubCodesStore subCodesStore = subCodeStore = new SubCodesStore(this);
	...
	private void manage(String str, int columnIndex, int day) {
		...
		try {
			...
			boolean isRoomFound;
			String[] roomAndCode = roomList.getRoomAndTeacherCode(tmp);
			if (roomAndCode != null) {
				room = roomAndCode[0];
				teacherCode = roomAndCode[1];
				isRoomFound = true;
			} else {
				room = tmp;
				teacherCode = tmp;
				isRoomFound = false;
				Log.w(TAG, "Room not found : " + tmp);
			}
	...
	private void manageTable(List<String> batchlist, char classType,
		...
		if (verifyFormat(batchlist, classType, subCode, room, teacherCode,
			...
			for (String batch : batchlist) {
				...
				String oldData = getOldData(String.valueOf(day), "c"
						+ databaseColumnIndex, batch, db);
	...
	private String getOldData(String row, String column, String table,
			SQLiteDatabase db) {
		String[] columns = { column };
		String result;

		Cursor cursor = db.query(table, columns, C_DAY + "='" + row + "'",
				null, null, null, null);
	...
	private boolean verifyFormat(List<String> batchlist, char classType,
			String subCode, String room, String teacherCode, boolean isRoomFound) {
		boolean result = true;
	...
	private void extractBatch(final String str, List<String> batch) {
		...
		tmp = str + ",";
		Log.d(TAG, tmp);
		tmp = tmp.replace(",", ",,");     //For rex matching
		p1 = Pattern.compile(",\\d+,");
		matcher = p1.matcher(tmp);
		while (matcher.find()) {

			String t = tmp.substring(matcher.start() + 1, matcher.end() - 1);
			int batNum = Integer.parseInt(t);
			
			String batchChar="";
			for (int i=matcher.start() ; i >=0 ; --i) {
				if(Character.isLetter(tmp.charAt(i))){
					batchChar = tmp.charAt(i)+"";
					break;
				}
			}
			if (batchChar.equals("")) continue;
			batch.add(batchChar + batNum);
			Log.i(TAG,"format A1,2,5  found" +batchChar+batNum);
		}
